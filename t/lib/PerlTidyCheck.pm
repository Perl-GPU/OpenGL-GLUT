...
sub find_untidied_files {
    ...
    my @slowest;
    my @untidied = map time_check( $_, \@slowest ), @perl;
    diag "slowest file with $slowest[0] seconds: $slowest[1]" if $slowest[0];
    return @untidied;
}
...
sub time_check {
    ...
    note sprintf "%s - $file", $duration;
    @{$slowest} = ( $duration, $file ) if !$slowest->[0] or $slowest->[0] < $duration;
    return @res;
}
